...
import { valueIsWrapped, Refs, unwrapValue, WrappedNext } from "./wrappedNext";
   ...
   if(this.operator) {
    ...
    this.operator = {
      call: (subscriber, source: Observable<any>) => {
        ...
        return originalOperator.call(new Subscriber({
          next: value => {
  ...
  if (
    ...
    this._subscribe = (subscriber: Subscriber<T>) => {
      ...
      subscriber.next = (value: T) => {
        ...
        originalNext.call(subscriber, wrapped);
      };
...
Observable.prototype.subscribe = function <T>(
  ...
  if (observerOrNext === null) {
    return subscribeWithPatch.call(this, observerOrNext, error, complete);
  }
  if (typeof observerOrNext === "function") {
    return subscribeWithPatch.call(
      this,
      (value: T) => {
  ...
  return subscribeWithPatch.call(this, {
    next:
      observerOrNext.next &&
      ((value: T) => {
        const unwrappedValue = unwrapValue(value);
...
export const unpatchedMap = <T, R>(mapFn: (value: T) => R) => (
  ...
  new Observable((obs) =>
    subscribeWithPatch.call(source$, {
